...
<tr class=" woocommerce-shipping-totals shipping">
    ...
    if ( 1 < count( $available_methods ) ) {
        printf(
            '<label><input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method" %4$s /> %5$s</label>',
            $index,
            esc_attr( sanitize_title( $method->id ) ),
            esc_attr( $method->id ),
            checked( $method->id, $chosen_method, false ),
            wc_cart_totals_shipping_method_label( $method ) // Используем label как текст внутри
        );
    } else {
        printf(
